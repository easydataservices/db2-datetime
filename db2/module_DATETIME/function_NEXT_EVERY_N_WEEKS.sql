...
ADD FUNCTION next_every_n_weeks(p_at_date DATE, p_base_date DATE, p_n_weeks SMALLINT) RETURNS DATE
  DETERMINISTIC
  NO EXTERNAL ACTION
BEGIN
  DECLARE v_n_days INTEGER;
  ...
  IF p_n_weeks NOT BETWEEN 1 AND 52 THEN
    SIGNAL SQLSTATE 'TZ002' SET MESSAGE_TEXT = 'P_N_WEEKS out of range';
  END IF;

  -- Handle NULL input, returning NULL.
  IF p_at_date IS NULL OR p_base_date IS NULL OR p_n_weeks IS NULL THEN
    RETURN NULL;
  END IF;
  ...
  SET v_n_days = p_n_weeks * 7;
  SET v_delta_days = DAYS(p_at_date) - DAYS(p_base_date);
  IF MOD(v_delta_days, v_n_days) = 0 THEN
    SET v_calculated_date = p_at_date;
  ELSE
    SET v_calculated_date = p_at_date + (v_n_days - MOD(v_delta_days, v_n_days)) DAYS;
  END IF;
