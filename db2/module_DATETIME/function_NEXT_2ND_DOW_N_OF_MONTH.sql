...
ADD FUNCTION next_2nd_dow_n_of_month(p_at_date DATE, p_dow_n SMALLINT, p_month SMALLINT) RETURNS DATE
  DETERMINISTIC
  NO EXTERNAL ACTION
BEGIN
  ...
  IF DAYOFWEEK_ISO(v_month_start_date) > p_dow_n THEN
    SET v_calculated_date = v_month_start_date + (7 + p_dow_n - DAYOFWEEK_ISO(v_month_start_date)) DAYS + 7 DAYS;
  ELSE
    SET v_calculated_date = v_month_start_date + (p_dow_n - DAYOFWEEK_ISO(v_month_start_date)) DAYS + 7 DAYS;
  END IF;
  ...
  IF v_calculated_date < p_at_date THEN
    RETURN next_2nd_dow_n_of_month(c_day_1 + YEAR(p_at_date) YEARS, p_dow_n, p_month);
  END IF;
